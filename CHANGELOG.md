...
- **`ar-mcp` follow-up on the #7c review (PR #24, merged before its review landed; refined again after the CodeRabbit + Codex review of the follow-up PR #55).** Hardened the MCP tool handlers in `scripts/mcp_server.py`:
  ...
  - **Durability + integrity hardening from the latest review round (CodeRabbit + Codex on PR #55, plus a dev-side Fable pass):**
    ...
    - Lower-severity follow-ups from the dev-side pass: `ar_aggregate` accepted with no explicit `--run` now pins the fresh verdict against the run dir it actually resolved (never surfacing a verdict for a different, concurrently-created run); a run killed between a move-aside and its settle leaves the prior at `verdict.json.prev`, which the next `ar_aggregate` adopts and restores; a rejected aggregate now names *why* (stale / malformed / unrecognized / wrong-run); a successful `ar_init` whose stdout can't be parsed falls back to the newest run rather than reporting `run_id: null`; and `authorized_by` is trimmed of surrounding whitespace before it enters the audit record.

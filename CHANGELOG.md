...
- **Negotiated-version match + dispatch-lock termination (E3-S2b review round 5).** From the re-review of round 4: a legacy `initialize` whose `MCP-Protocol-Version` header disagrees with the version the handshake will **negotiate** is now `400` — the round-4 guard compared the header to the raw body `protocolVersion`, so a body that **omits** `protocolVersion` (or sends an unsupported/non-string one) slipped through: it negotiates `SUPPORTED_PROTOCOLS[0]` while the response echoes the header, leaving the client pinned to an echoed version its own `Mcp-Session-Id` then `404`s on. The check now compares the header to the negotiated version (body version when supported, else `SUPPORTED_PROTOCOLS[0]`) (CodeRabbit r3945516733). **DELETE now terminates a session under `_HTTP_DISPATCH_LOCK`**, and the **GET stream commits its `200` under the same lock** (with the socket write timeout set *before* the header write so the lock is never held on a blocking write), so termination is atomic with request dispatch: a `DELETE` can no longer land after a POST's in-lock re-check but before `serve_message`, nor after a GET's in-lock validity re-check but before its `200` + `: connected` — closing the last check-to-commit windows in which a dead session could still be served or streamed (Codex r3945547146 / r3945547151).

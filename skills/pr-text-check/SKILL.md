...
3. **Docs shipped with the code.** `SKILL.md` / `README*` / `references/*.md` / `docs/` are
   updated for any behavior change *in this PR*, not a follow-up. A sensitive surface ships
   its committed threat model and its review verdict in the same PR.
...
7. **Commit messages are self-contained.** Subject in the imperative; body says *why*, not
   just *what*; each keepable commit is pushed the same turn it is made.

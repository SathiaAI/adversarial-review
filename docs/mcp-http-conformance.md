...
`tests/run_tests.py` holds a **curated** conformance manifest, `_S2D_CONFORMANCE`: one row per dispatch behavior mapping `behavior_id → stdio_test → http_test → spec clause`. The meta-test `t_mcp_s2d_conformance_manifest_covers_http` **fails CI if any referenced test is missing**, so adding a dispatch behavior forces HTTP coverage. It is curated on purpose — a name-grep of all `t_mcp_*` would false-positive on the ~80 pipeline/aggregate tests that are not transport conformance.

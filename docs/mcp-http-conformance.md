...
- The error is delivered **in-band on HTTP 200** (mirroring `UnsupportedProtocolVersion` `-32022`); non-200 stays reserved for transport-layer rejects (401/403/413). A modern **notification** POST (method present, no `id`) routes on `Mcp-Method` too, so enforcement applies — a violation is a normal in-band error with `id: null` (rather than the `202`-no-body a valid notification receives).
- `server/discover` (the version-agnostic probe) is **exempt**. Enforcement runs **after** Origin → auth → version → session, so it adds no new pre-auth disclosure; correct headers dispatch **byte-identically** to before.

Tests (all offline, HTTP transport): `t_mcp_http_modern_missing_mcp_method_is_header_mismatch`, `…_mcp_method_mismatch_…`, `…_tools_call_missing_mcp_name_…`, `…_mcp_name_mismatch_…`, `…_conflicting_mcp_method_…`, `…_notification_missing_method_header_is_mismatch`, `…_correct_routing_headers_dispatch`, and the legacy guard `t_mcp_http_legacy_needs_no_routing_headers`. Tracked as SAT-1115.

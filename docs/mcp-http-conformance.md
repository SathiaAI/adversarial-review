...
## Routing headers (2026-07-28 changelog #4) — enforced (SAT-1115)

The revision requires `Mcp-Method` (and `Mcp-Name` for `tools/call`) on Streamable-HTTP POSTs and defines `HeaderMismatch` (`-32020`). This server **enforces** them, **gated to the modern (`2026-07-28`) era** off the same `params._meta` version signal it already routes on — a **legacy** (≤2025-11-25) POST carries no routing headers and is untouched (this era-gating is the load-bearing invariant, guarded by `t_mcp_http_legacy_needs_no_routing_headers`). On a modern POST:

- `Mcp-Method` is **required** and must **equal** the body `method`; for `tools/call`, `Mcp-Name` is **required** and must **equal** `params.name`. Absent-when-required **or** a disagreeing value → `HeaderMismatch` (`-32020`), and **no tool runs** (enforcement precedes dispatch).
- Distinct-valued duplicate `Mcp-Method` / `Mcp-Name` headers are **rejected** as an ambiguous route; identical repeats from an intermediary are tolerated (values are whitespace-stripped) — the same convention as the existing `MCP-Protocol-Version` conflicting-header handling.
- For a **request** the error rides **in-band on HTTP 200** (mirroring `UnsupportedProtocolVersion` `-32022`; non-200 stays reserved for transport-layer rejects like 401/403/413) and echoes the pinned `MCP-Protocol-Version` like every other response, so a version-aware client does not misclassify it. A modern **notification** POST (method present, no `id`) is enforced too — but JSON-RPC gives a notification no reply, so a violation is **not dispatched** and answered with **`202` no-body**, exactly as a valid notification would be (never an error object with `id: null`).
- `server/discover` is exempt **only as the version-agnostic probe** — it carries no modern `_meta`, so it never reaches the era-gated block. A discover that **explicitly declares** the modern era *is* enforced like any modern POST and must carry `Mcp-Method: server/discover`, so the routing header stays consistent with the executed method for an intermediary. Enforcement runs **after** Origin → auth → version → session, so it adds no new pre-auth disclosure; correct headers dispatch **byte-identically** to before.

Tests (all offline, HTTP transport): `t_mcp_http_modern_missing_mcp_method_is_header_mismatch`, `…_mcp_method_mismatch_…`, `…_tools_call_missing_mcp_name_…`, `…_mcp_name_mismatch_…`, `…_conflicting_mcp_method_…`, `…_notification_header_violation_is_202_no_reply`, `…_discover_requires_mcp_method`, `…_header_mismatch_message_is_bounded`, `…_correct_routing_headers_dispatch`, and the legacy guard `t_mcp_http_legacy_needs_no_routing_headers`. Tracked as SAT-1115.

## Drift guard

`tests/run_tests.py` holds a **curated** conformance manifest, `_S2D_CONFORMANCE`: one row per dispatch behavior mapping `behavior_id → stdio_test → http_test → spec clause`. The meta-test `t_mcp_s2d_conformance_manifest_covers_http` **fails CI if a listed conformance test is renamed or removed** — a regression guard for the curated set. It **does not** auto-detect a brand-new dispatch behavior added without a row (there is no authoritative behavior registry to diff against), so adding a row for a new behavior is a manual, reviewed step. It is curated on purpose — a name-grep of all `t_mcp_*` would false-positive on the ~80 pipeline/aggregate tests that are not transport conformance. A row for an **HTTP-only** transport behavior (e.g. routing-header enforcement, which exists only on the Streamable-HTTP POST) carries `stdio_test = None`; the guard skips a `None` slot and still requires every named test to be callable.
